...
namespace PortableLib
{
    ...
    public class Win32
    {
        ...
        [DllImport("user32.dll")]
        public static extern IntPtr SetWindowsHookEx(HookTypes hookType, HookProc hookProc, IntPtr hInstance,
            int nThreadId);
        ...
        //Retrieve a handle to a window whose class name and window name match the specific strings
        ...
        public delegate bool EnumChildProc(IntPtr hwnd, Int32 lParam);
        ...
        [DllImport("gdi32.dll")]
        public static extern bool BitBlt(IntPtr hdcDst, int xDst, int yDst, int cx, int cy, IntPtr hdcSrc, int xSrc,
            int ySrc, uint ulRop);

        [DllImport("gdi32.dll")]
        public static extern bool StretchBlt(IntPtr hdcDst, int xDst, int yDst, int cx, int cy, IntPtr hdcSrc, int xSrc,
            int ySrc, int cxSrc, int cySrc, uint ulRop);
        ...
        public static extern int PathCompactPathEx(
            ...
            /* Pointer to a null-terminated string of max length (MAX_PATH) that contains the path to be altered */
            uint cchMax,
            /* Maximum number of chars to be contained in the new string, including the null character. Example: cchMax = 8, then 7 chars will be returned, the last for the null character. */
            uint dwFlags); /* Reserved */

        public static string PathCompactPathEx(string source, uint maxChars)
        {
            StringBuilder pszOut = new StringBuilder((int) MAX_PATH);
            StringBuilder pszSrc = new StringBuilder(source);

            int result = PathCompactPathEx(pszOut, pszSrc, maxChars, 0);
            if (result == 1)
                return pszOut.ToString();
            Debug.WriteLine("Win32.PathCompactPathEx failed to compact the path '" + source + "' down to '" + maxChars +
                            "' characters.");
            return string.Empty;
        }
